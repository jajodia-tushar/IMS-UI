    ...
    stages {
		...
		stage('Deploy'){
			steps {
				...
				sshCommand remote: remote, command: "docker service create --name ims-ui --network ims --publish 5002:80 --env-file ./env_ims_ui.list --mount ~/Resources:/Resources $DOCKER_REGISTRY/$DOCKER_REPO:$IMAGE_TAG"
